...
已实现（`web/src/components/StoryOutro.tsx`）：

- **结尾整块 CTA**，在内容读完之后才出现。访客看到的是「你的照片里也有这样一趟旅行」+ 去主页 / 免费做一篇；
  作者本人（会话 uid == stories.user_id）看到的是「回到我的故事」+ 主页 ——
  他回来多半是检查自己这篇的，下一步不是注册。
- **左上角轻量徽标**（半透明药丸）：中途划走的人同样需要一条路，
  不能只有读到底的人才有。仍然不挂全站导航 —— 顶上压一条产品导航
  会把作品变成"某个网站里的一页"。
